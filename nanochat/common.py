...
def get_base_dir():
    """
    Get the base directory for nanochat data and checkpoints.

    Priority:
    1. NANOCHAT_BASE_DIR environment variable (if set)
    2. /datadrive/nanochat (if /datadrive exists - for large storage)
    3. ~/.cache/nanochat (fallback for local development)
    """
    if os.environ.get("NANOCHAT_BASE_DIR"):
        nanochat_dir = os.environ.get("NANOCHAT_BASE_DIR")
    elif os.path.exists("/datadrive"):
        # Use /datadrive for large storage (training data, checkpoints)
        nanochat_dir = "/datadrive/nanochat"
    else:
        # Fallback to ~/.cache for local development
