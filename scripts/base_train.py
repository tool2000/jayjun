...
from nanochat.dataloader import (
    ...
    tokenizing_distributed_data_loader_bilingual_bos_bestfit,
    tokenizing_distributed_data_loader_bilingual_bos_bestfit_with_state,
)
...
parser.add_argument(
    ...
    type=int,
    default=42,
    help="Random seed for bilingual data mixing",
)
# Logging
parser.add_argument(
    "--log-every",
...
# Check for bilingual mode (from CLI or environment variable)
en_ratio = args.en_ratio
if en_ratio == 1.0:
    # Check environment variable as fallback
    env_ratio = os.environ.get("NANOCHAT_LANG_RATIO")
    if env_ratio is not None:
        en_ratio = float(env_ratio)

is_bilingual = en_ratio < 1.0
if is_bilingual:
    print0(
        f"Bilingual mode: {en_ratio * 100:.0f}% English, {(1 - en_ratio) * 100:.0f}% Korean"
    )
    train_loader = tokenizing_distributed_data_loader_bilingual_bos_bestfit_with_state(
        tokenizer,
        ...
        en_ratio=en_ratio,
        seed=args.bilingual_seed,
    )
    build_val_loader = lambda: tokenizing_distributed_data_loader_bilingual_bos_bestfit(
        tokenizer,
        args.device_batch_size,
        args.max_seq_len,
        split="val",
